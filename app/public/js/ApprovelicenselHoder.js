...
async function fetchLicenseHolders() {
  try {
    const response = await fetch(`/api/ApprovelicenselHoder?page=${currentPage}&pageSize=${pageSize}`);
    if (!response.ok) {
...
function nextPage() {
  ...
  document.getElementById('status').disabled = false; // Keep status field enabled

  // Change modal title
  document.getElementById('licenseHolderModalLabel').innerText = 'Cấp mới giấy phép lái xe';
